...
while(class(banana) == "try-error") banana <- try(tr.mu.sp(seqlen = 2000, meanmu = -6, covariance = 25e-6))
try(image(banana[[2]]))
...
while(class(pear) == "try-error") pear <- try(tr.mu.sp(seqlen = 2000, direct = T, meanmu = -6, covariance = 25e-6))
try(image(pear[[2]]))
...
while(class(apple) == "try-error") apple <- try(tr.mu.sp(seqlen = 2000, regcoefmu = 0, regcoefspr = 0, meanmu = -6, covariance = 25e-6))
try(image(apple[[2]]))
